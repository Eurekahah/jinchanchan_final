...
void BattleField::create_store() {
    ...
    store_button->setLocalZOrder(2);//设置显示顺序 试试行不行
}
...
void BattleField::Activate_timer() {
    ...
    int totalTime = 10;
    //float total_time = float(totalTime);
...
void BattleField::updateMoneyDisplay() {
    ...
    //auto money = PlayerManager::getInstance()->getPlayerByID(0)->getMoney();
    CCLOG("money===%d", money);
    money = PlayerManager::getInstance()->getPlayerByID(0)->getMoney();
    if (moneyLabel)
...
void BattleField::Display_Heroes_Backup(int num) {
    ...
    //drag_heroes();
    Director::getInstance()->getRunningScene()->addChild(hero);
    // 更新下一张图片的x坐标位置
...
void BattleField::removeAndUpdateSprite(Hero* image) {
    ...
    int i = 0;
    PlayerManager::getInstance()->getPlayerByID(0)->erase_synthesis_hero(image);
    auto heroList = PlayerManager::getInstance()->getPlayerByID(0)->getHeroList();
   /* for (Hero* h : heroList) {
...
void BattleField::synthesis(Hero* image) {
    ...
    for (const auto& heroInfo : heroList) {
        ...
        if (image->getHeroType() == heroInfo->getHeroType()) {
            num++;
        }
    }
    CCLOG("num=%d", num);
    if (num >= 3) {
        ...
        for (const auto& heroInfo : heroList) {
            ...
            if (image->getHeroType() == heroInfo->getHeroType() && time < 2) {
...
bool BattleField::init() {
    ...
    auto battlefield_image = Sprite::create("backgrounds/BattleImage.png");
    ...
    create_store();
    this->addChild(EquipmentBarforGame::create(20, 0));
    //roundBegin();
    ...
    */
    ...
    this->addChild(rune_factory);
    rune_factory->getTouchListener()->setEnabled(false);

    //初始化符文存储区
    RunesButton* rune_button = RunesButton::create();
    this->addChild(rune_button);
    RuneStore* rune_store = RuneStore::create();
    this->addChild(rune_store);
...
void BattleField::addPlayerHero(int playerId)
{
    ...
    if (!player->getHeroList().empty()) {
        ...
        {
            hero->stopAllActions();//停止所有英雄动画
            hero->removeFromParent();
            this->addChild(hero);
            //hero->setVisible(true);//让玩家英雄可见
        }
    }
    else {
...
#define doublebattle
#ifdef doublebattle
    PlayerManager::getInstance()->getPlayerByID(0)->getTouchListener()->setEnabled(true);//重新启用小小英雄监听器
...
#define DEBUG
    ...
    //覆盖layer停止所有行动 maybe除了英雄移动和商店？
    Director::getInstance()->getRunningScene()->addChild(StopMoveWhenBegin::create());
...
void BattleField::updateHero(int id)
{
    ...
    for (Hero*& hero : player->getHeroList())// TO DO:
    {
        ...
        CCLOG("health:%d", hero->getCurrentHealth());
        //如果是AI 则把所有都设置为不可见
        if (player->getIsAI() == true) {
...
void BattleField::roundOver(int winnerId, int loserId,int enemyId)
{
    ...
    //把layer拿掉
    auto layer=Director::getInstance()->getRunningScene()->getChildByName("stop_layer");
    if (layer) {
    ...
    for (const auto& player : PlayerManager::getInstance()->getPlayerList()) {
        if (player&&player->getRound() % 2 != 0) {
            allReadyToFashionShow = false;
            break;
        }

    }
    CCLOG("round:%d", winner->getRound());
    ...
    if (allReadyToFashionShow)
    {
        ...
        //奇数回合选符文 禁用小小英雄好了 然后休息的时候让它还在原来位置

        auto myself = PlayerManager::getInstance()->getPlayerByID(0);
        myself->getTouchListener()->setEnabled(false);
        myself->setPosition(Vec2(Player_init_pos, Player_init_pos));
...
void BattleField::GameOver()// TO DO:
{
    ...
    PlayerManager::getInstance()->updatePlayerList();
    /*auto exitButton = cocos2d::ui::Button::create("backgrounds/exit_image.png", "backgrounds/exit_image.png");
...
void BattleField::goToFashionShow()//选秀场景初始化
{
    ...
    auto playerList = PlayerManager::getInstance()->getPlayerList();
    ...
    // 在onEnter函数中创建计时器并启动
    schedule(CC_SCHEDULE_SELECTOR(BattleField::updatePlayerState), 5.0f);//初始化计时器
    schedule(CC_SCHEDULE_SELECTOR(BattleField::resumeAfterDelay), 10.0f);

}

void BattleField::resumeAfterDelay(float dt)
{
    unschedule(CC_SCHEDULE_SELECTOR(BattleField::resumeAfterDelay));
...
void BattleField::addAllaLivedPlayers()
{
    auto playerList = PlayerManager::getInstance()->getPlayerList();
...
void BattleField::presentEquipment()
{

    auto circle = SelectionLayer::create();

    this->addChild(circle);
    circle->random_create();
}

void BattleField::beginShow()
{
    auto playerList = PlayerManager::getInstance()->getPlayerList();
...
void BattleField::updatePlayerState(float dt) {
    ...
    if (currentPlayerIndex >= playerList.size()) {
        CCLOG("timer close.");
        unschedule(CC_SCHEDULE_SELECTOR(BattleField::updatePlayerState));
        currentPlayerIndex = 0;
        auto enemy = PlayerManager::getInstance()->getPlayerByID(1);
        removeChild(enemy);
        auto layer = getChildByName("selection_layer");
        removeChild(layer);
...
bool StopMoveWhenBegin::init() {
    if (!Layer::init()) {
        return false;
    }

    setLocalZOrder(2);//设置优先级
    setName("stop_layer");//设置名称
    // 注册触摸监听器
    auto touchListener = EventListenerTouchOneByOne::create();
    //能够吞没事件
    touchListener->setSwallowTouches(true);

    touchListener->onTouchBegan = CC_CALLBACK_2(StopMoveWhenBegin::onTouchBegan, this);
...
StopMoveWhenBegin* StopMoveWhenBegin ::create() {
    StopMoveWhenBegin* ret = new StopMoveWhenBegin();
    if (ret && ret->init()) {
        ret->autorelease();
        return ret;
    }
